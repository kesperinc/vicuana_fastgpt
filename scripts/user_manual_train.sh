...
PER_DEVICE_BATCH_SIZE=$((1024 * $GC_SCALE / $SEQ_LEN))
NUM_NODES=1
HOST_ADDR=127.0.0.1
...
torchrun \
  ...
  --node_rank=0 \
  chatserver/train/user_manual_train.py \
  --model_name_or_path /shared/llama_weights/hf-llama-${MODEL_SIZE}b \
  --data_path /home/haozhang/test-hao/ChatServer/data/faq.txt \
  --bf16 True \
  --output_dir /home/haozhang/test-hao/ChatServer/ckpts/ \
  --num_train_epochs 1 \
  --per_device_train_batch_size $PER_DEVICE_BATCH_SIZE \
  --per_device_eval_batch_size $PER_DEVICE_BATCH_SIZE \
  --gradient_accumulation_steps $((1024 / $SEQ_LEN / $PER_DEVICE_BATCH_SIZE / $NUM_NODES)) \
